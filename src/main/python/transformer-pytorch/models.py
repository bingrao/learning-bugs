...
class Transformer(nn.Module):
    ...
    def __init__(self, encoder, decoder):
        ...
        self.conv1 = nn.Conv2d(1, 1, kernel_size=(1, 3), stride=1, padding=(0, 1), bias=False)
        self.conv2 = nn.Conv2d(1, 1, kernel_size=(1, 3), stride=1, padding=(0, 1), bias=False)
        self.conv3 = nn.Conv2d(1, 1, kernel_size=(1, 3), stride=1, padding=(0, 1), bias=False)
    ...
    def forward(self, sources, inputs):
        ...
        # ==========================================================
        # test = torch.unsqueeze(memory, dim=1)
        # test = self.conv1(test)
        # test = self.leakyrelu(test)
        # test = self.conv2(test)
        # test = self.leakyrelu(test)
        # test = self.conv3(test)
        #
        # test = self.leakyrelu(test)
        # memory = memory + torch.squeeze(test)
